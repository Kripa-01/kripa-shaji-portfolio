            ...
            >
              {displayProjects.map((project, idx) => (
                ...
                >
                  <div 
                  className={
  `group relative rounded-2xl p-6 sm:p-8 md:p-10 border transition-all duration-500 w-full flex flex-col h-[520px] sm:h-[520px] md:h-[450px] overflow-hidden
   ${hoveredCard === idx ? 
      // MOBILE TAP FULL COLOR
      `bg-gradient-to-br from-gray-800/40 to-gray-900/40 backdrop-blur-xl border-gray-700/50 shadow-xl` 
      :
      // DEFAULT (DESKTOP + MOBILE INITIAL)
      "bg-gradient-to-br from-gray-800/40 to-gray-900/40 backdrop-blur-xl border-gray-700/50"
   }`
}

                    /* MOBILE TAP-HOVER FIX */
                    onMouseEnter={() => !isMobile && setHoveredCard(idx)}
                    onMouseLeave={() => !isMobile && setHoveredCard(null)}
                    onClick={() => isMobile && setHoveredCard(hoveredCard === idx ? null : idx)}

                    style={{
                      transform: hoveredCard === idx ? 'translateY(-8px) scale(1.02)' : 'translateY(0) scale(1)',
                      boxShadow: hoveredCard === idx 
                        ? `0 20px 60px -10px rgba(163, 230, 53, 0.3)` 
                        : 'none',
                  ...
                  >
                    ...
                    <div className="relative flex flex-col h-full z-10">
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <h3 className="text-2xl sm:text-3xl font-bold text-white group-hover:text-lime-400 transition-colors duration-300 leading-tight">
                          {project.title}
                        </h3>
                      </div>

                      {/* Role badge */}
                      <div className="inline-block bg-lime-400/10 border border-lime-400/30 rounded-lg px-4 py-2 mb-6 self-start backdrop-blur-sm group-hover:bg-lime-400/20 group-hover:border-lime-400/50 transition-all duration-300">
                      ...
                      {/* Description */}
                      <p className="text-gray-300 text-base sm:text-lg leading-relaxed mb-8 flex-grow group-hover:text-gray-200 transition-colors duration-300">
                        {project.description}
                      </p>
                      ...
                      <div className="space-y-3 mt-auto">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {project.technologies.map((tech, index) => (
                            <span
                              key={index}
                              className={`px-4 py-2 rounded-lg bg-gradient-to-r ${project.gradient} bg-opacity-10 border border-gray-600/50 text-white text-xs sm:text-sm font-medium hover:scale-110 hover:shadow-lg transition-all duration-300 cursor-pointer`}
                              style={{
          ...
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 sm:-translate-x-6 bg-gray-800/90 hover:bg-lime-400/20 border border-gray-700 hover:border-lime-400 rounded-full p-4 transition-all duration-300 hover:scale-110 group z-10 backdrop-blur-sm"
            aria-label="Prev"
          >
            <ChevronLeft className="w-7 h-7 text-gray-400 group-hover:text-lime-400 transition-colors duration-300" />
          </button>

          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 sm:translate-x-6 bg-gray-800/90 hover:bg-lime-400/20 border border-gray-700 hover:border-lime-400 rounded-full p-4 transition-all duration-300 hover:scale-110 group z-10 backdrop-blur-sm"
            aria-label="Next"
          >
            <ChevronRight className="w-7 h-7 text-gray-400 group-hover:text-lime-400 transition-colors duration-300" />
          </button>
            ...
            {Array.from({ length: maxIndex + 1 }).map((_, index) => (
              <button
                ...
                className={`transition-all duration-500 rounded-full relative overflow-hidden ${
                  index === currentIndex
                    ? 'w-14 h-3 bg-gradient-to-r from-lime-400 to-cyan-400 shadow-lg shadow-lime-400/50'
          ...
          <div className="mt-8 max-w-md mx-auto">
            <div className="h-1 bg-gray-800 rounded-full overflow-hidden">
              <div 
                className="h-full bg-gradient-to-r from-lime-400 to-cyan-400 transition-all duration-500 rounded-full"
